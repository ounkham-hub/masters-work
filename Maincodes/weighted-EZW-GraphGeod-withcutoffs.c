...
//Intialize matrices
   ...
   for(snap=1; snap<=maxsnap; snap++){
      ...
      sprintf(ofile,"c-EZW.O%d.H%d.wGraphGeod",snap,snap); 
      input=fopen(ifile,"r");
	     ...
	     if(currentmatrix[crow][0]!=0){
	        if(currentmatrix[crow][5]==55){ //Only water molecules
		      ...
		      weightmatrix[crow][4]=0.0;
	           ...
	           if(weightmatrix[crow][coln]<=0.900 && weightmatrix[crow][coln]!=0.000){ //If any O-H bond is less than 0.900, remove
		      currentmatrix[crow][0]=0;
		      currentmatrix[crow][1]=0;
		      currentmatrix[crow][2]=0;
		      currentmatrix[crow][3]=0;
		   ...
		   if(weightmatrix[crow][coln]<=0.700 && weightmatrix[crow][coln]!=0.000){ //If any O-H bond is less than 0.700. remove
        ...
        for(coln=2; coln<5; coln++){
           if(currentmatrix[crow][1]!=0){
              if(currentmatrix[crow][5]==1){ //for all eigens
                 if(weightmatrix[crow][coln]<0.900){ //if O-H bond has a weight less than 1.000, remove
                    currentmatrix[crow][0]=0;
                    currentmatrix[crow][1]=0;
                    currentmatrix[crow][2]=0;
                    currentmatrix[crow][3]=0;

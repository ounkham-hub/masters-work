...
//Read in list of unreactive H atoms and store into matrix
   ...
   PTinput=fopen("centroid-all-H-PT-events.txt","r");
   output=fopen("PTs-orig-Oatom-weights.txt","a");
   noutput=fopen("PTs-new-Oatom-weights.txt","a");
...
*/
		  ...
		  else if(PTmatrix[prow][1]!=ezwmatrix[erow][1]){ //If O atom doesnt match then it's the original O atom
/*
		      printf("%d %d %d %d %d %0.3f\n",
		      olines,
		      snap,

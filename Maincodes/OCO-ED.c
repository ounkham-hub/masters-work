...
//Parameters to read inputs
     ...
     int snap,OCO,Hyd1,Hyd2,Hyd3,maxsnap;
     int OCOrow,Hrow,countOCO,countzero;
     float float1,float2;
...
//Intialize counters
    ...
    maxsnap=50001; //Change accordingly!
...
//Loop over OCOarray to ensure there aren't more OCO's bonded to that H index
			  ...
			  if(OCOarray[OCOrow][0]!=OCOarray[z][0]){//If O's are different
			     if(OCOarray[OCOrow][1]==OCOarray[z][1]){ //and H's are the same
				if(holdarray[1][0]==0){ //for H with edge of 2
				   holdarray[1][0]=OCOarray[z][0];

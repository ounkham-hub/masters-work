...
//Loop and counter parameters
	...
	maxsnap=55001; //!!CHANGE ACCORDING TO SYSTEM!! The total number of snapshots!
/*
...
//	output8=fopen("ZZ.txt","a");
	output9=fopen("labeled-PT-events.txt","a");
	      ...
	      for(srow=1; srow<slines; srow++){
		    ...
		    if(startlist[srow][5]==2 && nextlist[nrow][5]==1){ //If initial eigen becomes Zundel
		       if(startlist[srow][1]!=nextlist[nrow][1]){// If OCO do NOT match
		          for(c=2; c<5; c++){
		             for(d=2; d<5; d++){
		                if(startlist[srow][c]==nextlist[nrow][d]){//if H's match
			           countEZ++;
			
				   fprintf(output9,"21 %d %d %d %d %d %d %d\n",
			  ...
			  for(c=2; c<5; c++){
			     for(d=2; d<5; d++){
			        if(startlist[srow][c]==nextlist[nrow][d]){
				   if(startlist[srow][c]!=0){
/*			              countEE++;
...
*/				   }
			  ...
			  for(c=2; c<5; c++){
			     for(d=2; d<5; d++){
			        if(startlist[srow][c]==nextlist[nrow][d]){//if H's matches
				   countZE++;

				   fprintf(output6,"12 %d %d %d %d %d %d %d\n",
				   startlist[srow][0], //initial snap
				   nextlist[nrow][0], //finial snap
...
//if Z dissociates, ONE of the participating OCO's will be in the next snap as an Eigen
			  for(c=2; c<5; c++){  
			     for(d=2; d<5; d++){ 
				   ...
				   fprintf(output9,"3 %d %d %d %d %d %d %d\n",
				   startlist[srow][0], //initial snap
				   nextlist[nrow][0], //finial snap
			  ...
			  for(c=2; c<5; c++){  
			     for(d=2; d<5; d++){ 
			        if(startlist[srow][c]==nextlist[nrow][d]){
			           countZdiss++;
/*
				   fprintf(output9,"ZD %d %d %d %d %d %d %d %d\n",
				   startlist[srow][0], //initial snap
				   nextlist[nrow][0], //finial snap
				   startlist[srow][1], //zOCO1
				   startlist[srow][6], //zOCO2
...
*/				}
		    ...
		    else if(startlist[srow][5]==1 && nextlist[nrow][5]==1){//If H from Z1 is shared with another Z2
		       if(startlist[srow][1]==nextlist[nrow][1] && startlist[srow][6]!=nextlist[nrow][1]){
//if one OCO from Z matches appears in a new Zundel structure - ZZ transfer
			  for(c=2; c<5; c++){
			     for(d=2; d<5; d++){
			        if(startlist[srow][c]==nextlist[nrow][d]){//if H's matches
				if(startlist[srow][1]!=nextlist[nrow][1] && startlist[srow][6]!=nextlist[nrow][1]){//If Zundels are different
/*				   countZZ++;

				   fprintf(output8,"ZZ %d %d %d %d %d %d %d %d %d\n",
				   startlist[srow][0], //initial snap
				   nextlist[nrow][0], //finial snap
				   startlist[srow][1], //1-zOCO1
				   startlist[srow][6], //1-zOCO2
				   nextlist[nrow][1], //2-zOCO1
				   nextlist[nrow][6], //2-zOCO2
				   startlist[srow][c], //H
				   startlist[srow][5], //initial structure
				   nextlist[nrow][5]); //final structure
*/				}
...
//if one OCO from Z matches appears in a new Zundel structure - ZZtransfer
			  for(c=2; c<5; c++){
			     for(d=2; d<5; d++){
			        if(startlist[srow][c]==nextlist[nrow][d]){//if H's matches
				if(startlist[srow][1]!=nextlist[nrow][1] && startlist[srow][6]!=nextlist[nrow][1]){
...
//if one OCO from Z matches appears in a new Zundel structure - Z-dissocation!
			  for(c=2; c<5; c++){
			     for(d=2; d<5; d++){
			        if(startlist[srow][c]==nextlist[nrow][d]){//if H's matches
				if(startlist[srow][1]!=nextlist[nrow][1] && startlist[srow][6]!=nextlist[nrow][1]){

...
//change in structure type in current snap to the next and includes
//weights.
//
//Adjusted to account for when structures do not exist, then the 
//lifetime is reset.
//
...
//Output: weighted-unreactive-water-1.0cutoff.txt
//
...
//Parameters to read O-H GraphGeods
   ...
   FILE *input,*nextinput,*output,*output2,*output3,*zoutput;
...
//      printf("%d %d %d\n",mainmatrix[y][0],mainmatrix[y][1],mainmatrix[y][2]);
   ...
   output=fopen("eigen-1.000E-1.000Z-cutoff.txt","a"); //@
   zoutput=fopen("zundel-1.000E-1.000Z-cutoff.txt","a"); //#
	...
	for(coln=2; coln<5; coln++){
	   if(currentmatrix[crow][1]!=0){
	      if(currentmatrix[crow][5]==55){ //for all waters
		    ...
		    currentweight[crow][2]=0.0;
		    currentweight[crow][3]=0.0;
		    currentweight[crow][4]=0.0; 
	         }  
	      }
	      else if(currentmatrix[crow][5]==1){ //for all eigens
		 if(currentweight[crow][coln]>1.000){ // @ 
//Leave as Eigen
		 }
		 else if(currentweight[crow][coln]<1.000){ //@ Structure is more water-like, remove
		    currentmatrix[crow][0]=0;
		    currentmatrix[crow][1]=0;
		    currentmatrix[crow][2]=0;
		    currentmatrix[crow][3]=0;
...
//Leave as Zundel
		 ...
		 else if(currentweight[crow][coln]<=1.000){ // # change to Eigen
	...
	for(coln=2; coln<5; coln++){
	   if(currentmatrix[crow][1]!=0){
	      if(currentmatrix[crow][5]==1){ //for all eigens
	         if(currentweight[crow][coln]<1.000){ // @ if O-H bond has a weight less than 1.000, remove
	     	    currentmatrix[crow][0]=0;
		    currentmatrix[crow][1]=0;
		    currentmatrix[crow][2]=0;
		    currentmatrix[crow][3]=0;
	...
	for(ncoln=2; ncoln<5; ncoln++){
	   if(nextmatrix[nrow][1]!=0){
	      if(nextmatrix[nrow][5]==55){ //for all waters
		    ...
		    nextweight[nrow][2]=0.0;
		    nextweight[nrow][3]=0.0;
		    nextweight[nrow][4]=0.0; 
	         }  
	      }
	      else if(nextmatrix[nrow][5]==1){ //for all eigens
		 if(nextweight[nrow][ncoln]>1.000){ // @ 
//Leave as Eigen
		 }
		 else if(nextweight[nrow][ncoln]<1.000){ // @ Structure is more water-like, remove
		    nextmatrix[nrow][0]=0;
		    nextmatrix[nrow][1]=0;
		    nextmatrix[nrow][2]=0;
		    nextmatrix[nrow][3]=0;
...
//Leave as Zundel
		 ...
		 else if(nextweight[nrow][ncoln]<=1.000){ // # change to Eigen
		    nextmatrix[nrow][5]=1;
//Loop through and change partner flag to Eigen too
		    for(y=0; y<nlines; y++){
	 	       for(z=2; z<5; z++){
...
//Remove all Eigens that are more water-like and have an O-H bond <0.700
	...
	for(ncoln=2; ncoln<5; ncoln++){
	   if(nextmatrix[nrow][1]!=0){
	      if(nextmatrix[nrow][5]==1){ //for all eigens
	         if(nextweight[nrow][ncoln]<1.000){ //@ if O-H bond has a weight less than 1.000, remove
	     	    nextmatrix[nrow][0]=0;
		    nextmatrix[nrow][1]=0;
		    nextmatrix[nrow][2]=0;
		    nextmatrix[nrow][3]=0;
...
//******If snapshot is NOT consecutive, structure is not in GraphGeod file 
		     if(mainmatrix[mrow][4]==nextmatrix[nrow][0]-1){ //if final snapshot is equal to previously analyzed snapshot
	                mainmatrix[mrow][4]=mainmatrix[mrow][4]+1; //update final snapshot           
                     }
		     else if(mainmatrix[mrow][4]!=nextmatrix[nrow][0]-1){ //If snapshot is NOT consectutive
	                diff=mainmatrix[mrow][4]-mainmatrix[mrow][3];
                     
		        if(diff>1){
			   if(mainmatrix[mrow][2]==2){ //For waters (55) or eigens (1)
//Print initial and final snap. water appeared
	                      fprintf(zoutput,"%d %d %d %d %d %d\n",
		              mainmatrix[mrow][0],
		              mainmatrix[mrow][1],
		              mainmatrix[mrow][2],
		              mainmatrix[mrow][3],
			   ...
			   else if(mainmatrix[mrow][2]==1){ //For waters (55) or eigens (1)
//Print initial and final snap. water appeared
...
//Update structure and time info for O index
		     ...
		     mainmatrix[mrow][4]=nextmatrix[nrow][0]; //restart final snap
 	          ...
 	          else if(mainmatrix[mrow][2]==1 && nextmatrix[nrow][5]!=1){ //change in structure!
//Calculate the lifetimes
	             diff=mainmatrix[mrow][4]-mainmatrix[mrow][3];

...
//Setting up parameters
   ...
   int lines,numzundels;
   FILE *input,*output;
   char ifile[100]; 

      output=fopen("total-zundel-vs-time.txt","a");

   for(snap=1; snap<=50001; snap++){

      sprintf(ifile,"Z-HCl.input.O%d.xyz.H%d.xyz.GraphGeod",snap,snap);
      input=fopen(ifile,"r");
      lines=0;
      ...
      if(lines==0){//if no Eigens
          ...
          numzundels=lines/6; //6 O-H bonds per Zundel (two OCO's)
          fprintf(output,"%d %d\n",snap,numzundels);
      }	
    }//end snap-loop
    fclose(output);

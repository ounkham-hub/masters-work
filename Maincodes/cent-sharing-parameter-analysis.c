...
//*************************************************************************
//Program created to calculate the sharing parameter between the two
//shared bonds within a Zundel, First, Zundels are identified using 
//coordination number and then the distance (O1H-O2H) are calculated.
//
...
//      coln 3: O1H distance
//      coln 4: O2H distance
//      coln 5: O1H-O2H = sharing parameter
//     
//
...
int main(){
   ...
   float whold,water,eigen,zundel,dist;
   FILE *input,*zoutput;
   char ifile[100],ofile4[100];

   int CBmatrix[1000][20];
   int newmatrix[1000][7];
   float distmatrix[1000][20];
   float newdist[1000][7];



   zoutput=fopen("total-zundel-delta-results.txt","a");
	    ...
	    if(Oindex==Oxy){ //if O index matches the previous one, store info
	       if(Hhold!=Hyd){
	          if(countOindex==2){
	             ...
	             distmatrix[clines][colns]=whold;
		     colns++;
		   }
		   CBmatrix[clines][0]=snap;
		   CBmatrix[clines][1]=Oxy;
		   CBmatrix[clines][colns]=Hyd;
		   distmatrix[clines][colns]=dist;
		   Oindex=Oxy;
		   Hhold=Hyd;
		   whold=dist;
		   colns++;
...
/*    bcount=0;
//Cross check to see if H indices and associated dists matches
...
//*************************************************************************
   ...
   for(crow=0; crow<clines+1; crow++){
	 ...
	 if(CBmatrix[crow][bcoln]!=0 && distmatrix[crow][bcoln]!=0.000){ //for nonzero elements
	    newmatrix[nlines][0]=snap;
	    newmatrix[nlines][1]=CBmatrix[crow][1];
	    newmatrix[nlines][CNcount]=CBmatrix[crow][bcoln];
	    newdist[nlines][CNcount]=distmatrix[crow][bcoln];
	    CNcount++;
	 }
      }//end bcoln-loop
...
/*
   for(x=0; x<nlines; x++){
      ...
      for(y=0; y<6; y++){
         if(newdist[x][y]!=0.000){
	    printf("%0.3f ",newdist[x][y]);
         }
      }
      printf("\n");
   }
...
//user-input definitions (dist and CN) across all OH bonds.
//*************************************************************************
...
//Print out dists between shared proton and water molecule
    ...
    newdist[nrow][ncoln], //O1-H* dist
    newdist[y][z]); //O2-H* dist
